...
parser.add_argument('--source2tar', default='visdac_source2target', type=str)
...
def eval_and_label_dataset(epoch, model, banks):
    ...
    matrix = confusion_matrix(gt_labels.to('cpu'), pred_labels.to('cpu'))
    acc11 = matrix.diagonal() / matrix.sum(axis=1) * 100
    aacc = acc11.mean()
    aa = [str(np.round(i, 2)) for i in acc11]
    acc_str = ' '.join(aa)
...
load_weights(net, 'logs/' + args.source2tar + '/weights_best.tar')  #args.source
load_weights(momentum_net, 'logs/' + args.source2tar + '/weights_best.tar')
